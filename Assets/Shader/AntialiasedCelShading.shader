...
/// 
Shader "Unfettered/Antialiased Cel Shading" {
	Properties {
	...
	SubShader {
		...
		*/
		Pass {
			...
			sampler2D _MainTex;  //texture unit
			float4 _MainTex_ST;
			...
			fixed4 frag(v2f i) : SV_Target { 
				...
				fixed w = fwidth(diff) * 2.0;  //checks if the current pixel is within an epsilon (w in the code) of the hard edge then do smoothly transition
